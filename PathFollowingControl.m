...
isReached = false ;     % bool to check the ship reached the destination
xEarthHistory = [] ;    % container to draw trajectory 
yEarthHistory = [] ;    % container to draw trajectory


%% Simulation running till the ship reaches the destination
while ~isReached
    %% Advance direction angle error
    errorX = xDestination - xEarth ;        % error of x between the target and the ship
    errorY = yDestination - yEarth ;        % error of y between the target and the ship
    ...
    hold on ;
    plot(yDestination, xDestination, 'vr') ;        % destination
